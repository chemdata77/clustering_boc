...
db_ir = connect('/home/qzh/data/qm9_ir_spectrum.db')
db_qm9 = connect('/home/qzh/data/qm9.db')
...
with open('/home/qzh/data_pre/meanshift_0.015q/qm9_id_boc_ms.lst', 'rb') as fp:
    c = pickle.load(fp)
...
if __name__ == '__main__':
    ...
    # vali dataset
    vali_boc_lst, vali_tgt_lst = boc_lst_all[int(train_ratio * len(rows_ir)):int(
        (train_ratio + vali_ratio) * len(rows_ir))], tgt_lst_all[int(train_ratio * len(rows_ir)):int(
        (train_ratio + vali_ratio) * len(rows_ir))]
    ...
    y_pred = np.array(np.clip(np.squeeze(np.array(y_pred) / (y_pred.max(axis=1).reshape(-1, 1))), 0, 1))
    ...
    for i in range(y_data.shape[0]):
        ...
        np.savetxt(str(i + moving) + '.txt', save_data)
